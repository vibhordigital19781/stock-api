...
def favicon_ico():
    ...
    return FileResponse(p, media_type="image/x-icon") if os.path.exists(p) else JSONResponse({}, status_code=404)

@app.get("/favicon.svg")
def favicon_svg():
    import os
    p = os.path.join(os.path.dirname(__file__), "favicon.svg")
    return FileResponse(p, media_type="image/svg+xml") if os.path.exists(p) else JSONResponse({}, status_code=404)

@app.get("/apple-touch-icon.png")
def apple_touch_icon():
    import os
    p = os.path.join(os.path.dirname(__file__), "apple-touch-icon.png")
    return FileResponse(p, media_type="image/png") if os.path.exists(p) else JSONResponse({}, status_code=404)

@app.get("/site.webmanifest")
def webmanifest():
    import os
    p = os.path.join(os.path.dirname(__file__), "site.webmanifest")
...
def offline_page():
    ...
    import os
    p = os.path.join(os.path.dirname(__file__), "offline.html")
    if os.path.exists(p):
        return FileResponse(p, media_type="text/html")
